...
public class DudeNotFull extends Moveable implements Transformable, Executeable{

    private int resourceLimit;
    public DudeNotFull(String id, Point position, List<PImage> images, int health, int actionPeriod, int animationPeriod,int resourceLimit) {
    ...
    public boolean transform(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        if (this.getResourceCount() >= this.resourceLimit) {
            DudeFull miner = new DudeFull(this.getid(),
                    this.position,
                    this.getimages(),
                    0,
                    this.getActionPeriod(),
